...
ActionResult< CTFBot >	CTFBotAttack::Update( CTFBot *me, float interval )
{
		 ...
		 !me->IsLineOfFireClear( threat->GetEntity()->EyePosition() ) )
	{
		if ( threat->IsVisibleRecently() )
		{
			if ( isUsingCloseRangeWeapon && !me->IsBotMannVsMachinePopulator())	// all bots in MvM use the default route
			{
		...
		else
		{
			...
			if ( m_repathTimer.IsElapsed() )
			{
				...
				{
					CTFBotPathCost cost( me, DEFAULT_ROUTE );
					float maxPathLength = me->IsBotMannVsMachinePopulator() ? TFBOT_MVM_MAX_PATH_LENGTH : 0.0f;
